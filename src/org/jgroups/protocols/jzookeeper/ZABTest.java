...
public class ZABTest extends ReceiverAdapter {
	...
	final AtomicLong seqno = new AtomicLong(-1); // monotonically increasing
													// seqno, to be used by all
													// threads
	private final Map<MessageId, Stats> latencies = Collections
			.synchronizedMap(new HashMap<MessageId, Stats>());
	// private final Map<MessageId,Stats> latencies = new
	// HashMap<MessageId,Stats>();

	protected static final short ID = ClassConfigurator
			.getProtocolId(MMZAB.class);
	private AtomicLong localSequence = new AtomicLong(); // This nodes sequence
															// number
	...
	public void viewAccepted(View new_view) {
		...
		if (mbrs.size() == 3) {
			...
			System.out.println("[" + local_addr + "]" + "Zab box view = "
					+ zabBox);
	...
	public void sendMessages(long mesNums, int mesSize, int num_threads) {
		final AtomicInteger actually_sent = new AtomicInteger(0); // incremented
																	// *after*
																	// sending a
																	// message
		final AtomicLong seqno = new AtomicLong(1); // monotonically increasing
		...
		for (int i = 0; i < num_threads; i++) {
			senders[i] = new Sender(barrier, mesNums, localSequence,
					actually_sent, seqno, payload);
	...
	public void receive(Message msg) {
		...
		synchronized (this) {
			if (testHeader.getType() != ZABHeader.START_SENDING) {
				...
				System.out.println("message received = zxid  is = "
						+ testHeader.getZxid() + " " + msgReceived + " at "
						+ getCurrentTimeStamp());
				...
				// "Received START_SENDING "+ getCurrentTimeStamp());
				msgReceived = 0;
		...
		while (true) {
			try {
				...
				case '1':
					...
					ZABHeader startHeader = new ZABHeader(
							ZABHeader.START_SENDING, 1, mid);
	...
	public class Sender extends Thread {
		...
		protected Sender(CyclicBarrier barrier, long numsMsg, AtomicLong local,
				AtomicInteger actually_sent, AtomicLong seqno, byte[] payload) {
			...
			for (int i = 0; i < numsMsg; i++) {
				...
				try {
					MessageId messageId = new MessageId(local_addr,
							local.getAndIncrement()); // Increment localSequence
